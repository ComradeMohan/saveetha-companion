...
export default function JobsPage() {
    ...
    const [allJobs, setAllJobs] = useState<TransformedJob[]>([]);
    const [visibleJobs, setVisibleJobs] = useState<TransformedJob[]>([]);
    ...
    const filteredJobs = useMemo(() => {
        ...
        return allJobs.filter(j => 
            ...
            j.jobType.toLowerCase().includes(lowercasedFilter)
        );
    }, [searchTerm, allJobs]);
            ...
            return (
                <Card className="max-w-md mx-auto text-center">
                    <CardHeader>
                        ...
                        <CardContent>
                            <p className="mb-4">You must be logged in to view remote jobs.</p>
                            <Button asChild>
                                ...
                                <div className="p-6 flex flex-col sm:flex-row gap-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted-foreground mb-3">
                                            ...
                                            <div className="flex items-center gap-1.5 text-sm text-muted-foreground"><MapPin className="h-4 w-4"/> {job.location}</div>
                                    </div>
                                </div>
                                    <CardFooter className="bg-muted/50 p-3 flex justify-between items-center">
                                    <div className="text-xs text-muted-foreground">
                                        Posted {formatDistanceToNow(new Date(job.publicationDate), { addSuffix: true })}
                                    </div>
            ...
            <main className="flex-1 pt-20 pb-12 md:py-16">
                <div className="container mx-auto px-4 max-w-4xl">
                    <div className="text-center mb-10">
                        <h2 className="text-3xl font-bold tracking-tight">Remote Jobs</h2>
                        <p className="text-muted-foreground mt-2">
                            Explore remote job opportunities from various companies.
                        </p>
                    </div>
                    {renderContent()}
                </div>
