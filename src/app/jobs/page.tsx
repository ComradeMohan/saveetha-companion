...
export default function JobsPage() {
    ...
    const fetchJobs = useCallback(async () => {
        ...
        try {
            ...
            setVisibleJobs(data.jobs.slice(0, JOBS_PER_PAGE));
            setCanLoadMore(data.jobs.length > JOBS_PER_PAGE);
        } catch (error) {
    ...
    useEffect(() => {
        setVisibleJobs(filteredJobs.slice(0, JOBS_PER_PAGE));
        setCanLoadMore(filteredJobs.length > JOBS_PER_PAGE);
    }, [filteredJobs]);
    

    const handleLoadMore = () => {
        const currentLength = visibleJobs.length;
        const nextJobs = filteredJobs.slice(currentLength, currentLength + JOBS_PER_PAGE);
        setVisibleJobs(prevJobs => [...prevJobs, ...nextJobs]);
        if (currentLength + JOBS_PER_PAGE >= filteredJobs.length) {
